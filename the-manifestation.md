...
so, because I have to be this way, I've learned to just go with the flow and realise that I'm always going to be the "last" one to get there. you see things from the (potential) perspective of the first; I however, see things from the perspective of the last. it'll only happen when it's so inevitable that everyone in resistance it is forced to allow it, or not survive. it's basically a new solar regime, so I do enjoy the thoughts, but I see it from another perspective, which is that it's gatta be smoothe.
