...
currently I'm looking at this layer as a sort of gradient of auras (subject to change, but I'm drawing on the conceptual mode for this), but what's important to note here that in this view it's easy to see how the different things interact with each other and therefor, at a glance, I can see how stable environment is. I don't want have concepts/machines/parts of the factory causing negatively affecting the interests of other sections/parts of the island. everything needs to well compartmentalised (the layer above) and also it needs to have a sort of balance to it.

this perception of the whole "ecosystem" as a system of balances, and so here there all of the stabilising forces need to be setup so that  that influence the concepts to remain in a balanced state. here, you'll have all of your conditions to notice that something is getting out of balance and then some logic to perform to get the system back into balance. I'll have to explain this in further detail after explaining the other levels layers above this one, but one can see that its main method of stability will have its effect mostly on reading and acting on values present in the schematic layer (4d), and then (probably) taking action in the above symbolic layer (and perhaps but I'm not 100% sure how to visualise this) in the conceptual.

it's important to note here at this point that the 4d, 6d, and 8d layers are mostly informational layers. not much action is made here. actual things are done in 3d (making machines and moving things around), 5d (modifying behaviour abstractly by affecting the auras), 7d (rearranging the parts/symbols and their calculations/formulas interactions), and 9d (changing the desired output for the different parts/symbols).

the one after that is symbolic (7d). this is where things lose their physicality. now, we're just sorta looking at block diagrams, and the way that these different parts interact with each other. each section of the factory has a set of inputs and outputs and they interface with other parts through something like pipes. these pipes have a symbolic formula or algorithm to them and can even be modeled with backpressure and the like. this view should be pretty similar to the code which moves the physical resources around on the belts and pipes.

another way something like a spreadsheet which is defining symbolic groups and making sure that under alarming conditions, it'll make an action in response.
