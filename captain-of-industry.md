...
it'd also be cool to integrate these things into a sort of GUI something like the old VB6 inteface... add a button and set the click action to do one of those things. what I want to be able to do is to create my own control panels for the factories. it'd look like the existing material lmyout buttons that captain already has, but they have the text I put in the button and performs the action that I want. also some visual indicators like progress bars and text labels, sliders, etc. (all the stuff that's already there)
