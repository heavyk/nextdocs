...
### 2023-03-08 17:38 - a corkscrew idea

@[youtube](https://youtu.be/TwEWfWsKQPs?list=TLPQMDgwMzIwMjMxC5gI8e-JRg)

this poem reminded me of how an idea sort of works. imagine it like a corkscrew of 720(degrees): one full turn tto the left, and one full turn to the right. you start at one extreme (1) and go all the way to the opposite extreme (14). this is the same concept, but expressed in some multiple of it's angle: `-360*(x/7)+(x/7)*360`.

the poem starts out with the absolute most positive expression of the idea, then spins around to normal at the cuff between (7|8).

one way to look at the idea is to notice the spectrum of youthful perception. it starts out with: "can't wait", "one elusive thing makes everything perfect", etc. -- then goes to last efforts to get what I think I want (7, defined in 1), crossing the threshold into a sort of giving up and preparing for the worst (8, down till 14). now, nothing that I wanted (defined in 1) is how I want it, so I'll be a bit bitter and upset about it, but I know it has to be that way because (defined at 1) isn't possible...
